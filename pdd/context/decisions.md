...
**Opt-in exercise library download (2026-06-30, `exercises-dataset-snappet`).** The bundled catalog ships 873 exercises with no images; the hasaneyldrm/exercises-dataset has 1,324 exercises with animated GIF demos — a quality jump worth surfacing. **Decision: mirror the Kilter catalog pattern.** The data is hosted on `harshal2802/Snappet` (exercises-dataset/data/exercises.json; images/; videos/). The mobile app ships no extra data; an opt-in "Download exercise library" banner in the Library section fetches the JSON (~800 KB) at the user's explicit request. **Narrow network exception** (same posture as the Kilter catalog, decisions.md 2026-06-05): one user-initiated GET to the Snappet web repo; no analytics, no background sync, no user data uploaded; health + media still never leave the device. **GIF display via Safari link** (not inline): inline GIF animation requires a third-party library or a custom UIViewRepresentable + GIF decoder — overkill for v1; a Safari link surfaces the animated demo with zero dependencies. Inline GIF is a tracked follow-on. **IDs prefixed `ext-`**: avoids collision with the 873 bundled exercises (underscore-slug ids). **No new SwiftData model**: the downloaded catalog is a plain JSON file managed by `ExerciseLibraryStore`, keeping the schema version unchanged. The conversion script (`experiments/exercises-convert/convert.py`) transforms the source format to Snappet's Exercise schema; it must be run once in the web repo to produce the hosted file.
