...
- **#92 Room v4 → v5 is one self-contained additive AutoMigration.** Three NULLABLE HR columns
  ...
  `@AutoMigration(4, 5)` — nothing is touched, nothing can be lost. **Cross-wave note:** Wave 2 also
  bumps to v5 independently (workout columns) in a separate worktree; the reviewer renumbers ONE
  migration to v5→v6 at merge. The column-set + AutoMigration are clearly commented (`KilterSession`,
  `SnappetDatabase`) so that's a one-line change. `MigrationBaselineTest` gains a `runMigrationsAndValidate`
  case proving the v4 row survives with the new columns null.
