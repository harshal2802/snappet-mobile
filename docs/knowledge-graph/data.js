...
const nodes = [
  ...
  { id: "studio-composer", label: "StudioComposer", type: "service", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Services/StudioComposer.swift", desc: "Multi-clip render engine for the full studio, generalizing VideoStudio from one clip to a StudioProjectSnapshot timeline: ordered video clips inserted sequentially (trim + speed) on a shared output canvas, reused for preview + export. Three export paths by feature: no-filter clips use a single-track per-clip orientation+crop layer instruction; any colour filter (S2) routes through a Core Image compositor (AVMutableVideoComposition applyingCIFiltersWithHandler → StudioFilters); any dissolve transition (S3) uses a two-track path (clips alternate A/B, B opacity-ramped over each overlap from the StudioGeometry timeline). Device-verified (S0–S3 spike): stitch ~free, transform ~0.2x realtime, +filter ~15%, dissolve ~2.8s/16s. Pure placement/keyframe math is StudioGeometry. Slide/zoom transitions, filter+transition combination, overlays, Ken-Burns photos remain follow-ups.", tags: ["avfoundation","coreimage","studio","multi-clip","filters","transitions","device-only"] },
  ...
  { id: "studio-overlays", label: "StudioOverlays", type: "service", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Services/StudioOverlays.swift", desc: "S4 text-overlay renderer: builds a Core Animation layer tree (CATextLayer per OverlayItem — positioned via ClipEditGeometry.layerPoint, scaled/rotated/coloured, time-gated to [startSec,endSec] by an opacity keyframe) composited through AVVideoCompositionCoreAnimationTool. Attached by StudioComposer on the instruction-based paths (no-filter / transition). Device-verified export ~4.9s/16s. Sticker overlays + keyframed opacity + overlay-with-filter are follow-ups.", tags: ["coreanimation","overlays","text","studio","device-only"] },
