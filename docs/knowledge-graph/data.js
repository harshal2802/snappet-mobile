...
const nodes = [
  ...
  { id: "studio-timeline", label: "StudioTimelineView", type: "component", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/StudioTimelineView.swift", desc: "The edits/CapCut scrubbable timeline: clip strips laid out by output duration, a fixed centre playhead, a time ruler, tap-to-select, and drag-to-trim handles (leading→trimStart, trailing→trimEnd) on the selected clip. Dragging the strip scrubs (seeks the preview + moves the playhead); during playback it auto-advances (offset by vm.currentTime). Trim commits once on drag-end (one undo entry + one rebuild). Strips are coloured placeholders — thumbnail strips are a device-only follow-up.", tags: ["swiftui","timeline","scrub","trim","playhead","studio"] },
