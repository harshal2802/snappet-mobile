...
const nodes = [
  ...
  { id: "kilter-detail", label: "KilterClimbDetailView", type: "screen", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterClimbDetailView.swift", desc: "A climb on the full board grid (faint holes + role-colored holds), an angle picker, a per-angle grade chart, a Classic (benchmark) badge + FA, Flash/Sent/Project/Attempt logging, a Saved toggle, a beta link, and gated BLE illumination with a clear connect flow (spinner + Cancel while connecting, retry on failure, Bluetooth-off / permission guidance).", tags: ["board","render","logging"], shot: "../screenshots/kilter-detail.png" },
  ...
  { id: "kilter-board-ctrl", label: "KilterBoardController", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterBoardController.swift", desc: "CoreBluetooth controller that scans, connects and writes Aurora illumination packets, gated behind an explicit Connect tap. Discovers boards by advertised name (Aurora boards don't advertise the service UUID), guards scan + connect + discovery with a timeout watchdog so a missing/asleep board can't wedge the UI on 'Connecting…', and exposes distinct states (bluetoothOff / unauthorized / failed-with-message) plus cancel() so the user is never trapped. DEVICE-UNVERIFIED — the GATT UUIDs + wire format are community-reverse-engineered and not confirmed on hardware. Opens a session on connect.", tags: ["ble","corebluetooth","unverified","gated","timeout"] },
