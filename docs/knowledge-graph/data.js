...
const nodes = [
  ...
  { id: "pomodoro-root", label: "PomodoroRootView", type: "screen", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/Snappet/Features/Pomodoro/PomodoroRootView.swift", desc: "The timer face + a 7-day focus chart. Pushes History; presents Settings. References AppModel.pomodoroTimer (not @State) so the session survives navigating away and back.", tags: ["root","timer"], shot: "../screenshots/04-pomodoro.png" },
  ...
  { id: "pomodoro-notifications", label: "PomodoroNotifications", type: "service", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/Snappet/Services/PomodoroNotifications.swift", desc: "Schedules a UNCalendarNotificationTrigger at the wall-clock end of each phase so a backgrounded / locked user still learns the phase ended. Cancels on pause/reset. Pure phaseEndContent is unit-tested.", tags: ["notifications","background","pure","tested"] },
  { id: "pomodoro-live-activity-ctrl", label: "PomodoroLiveActivityController", type: "service", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/Snappet/Services/PomodoroLiveActivityController.swift", desc: "Owns the Activity<PomodoroActivityAttributes> lifecycle: push() starts or updates (phase transitions in place), end() ends on stop/reset. No-ops where ActivityKit is unavailable/unauthorized.", tags: ["activitykit","live-activity"] },
  { id: "pomodoro-activity-attributes", label: "PomodoroActivityAttributes", type: "widget", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/Shared/PomodoroActivityAttributes.swift", desc: "ActivityAttributes contract for a Pomodoro session (phaseEndDate / phaseLabel / paused), compiled into both the app and the widget extension. No static attributes — all state is in ContentState because the phase label changes on each transition.", tags: ["shared","activitykit","contract"] },
  { id: "pomodoro-live-activity-widget", label: "PomodoroLiveActivity", type: "widget", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/SnappetWidgets/PomodoroLiveActivity.swift", desc: "Widget that renders the Pomodoro session Live Activity on the Lock Screen + Dynamic Island: phase label (Focus / Break, tinted red / green) and an OS-ticked countdown to phase end via Text(timerInterval:countsDown:).", tags: ["widget","live-activity","dynamic-island","countdown"] },
  { id: "pomodoro-focus-banner", label: "PomodoroFocusBanner", type: "section", group: "pomodoro", category: "productivity", platform: "ios",
    file: "ios/App/Snappet/Features/AppLibrary/AppLibraryView.swift", desc: "Re-entry chip shown at the top of the App Library while app.pomodoroTimer.isRunning. Shows remaining time and navigates back into the module on tap. Animates in/out with .snappy.", tags: ["banner","background","re-entry"] },
...
const links = [
  ...
  { source: "appmodel", target: "pomodoro-notifications", type: "contains" },
  { source: "appmodel", target: "pomodoro-live-activity-ctrl", type: "contains" },
  ...
  { source: "applibrary", target: "pomodoro-focus-banner", type: "contains" },
  { source: "pomodoro-focus-banner", target: "m-pomodoro", type: "navigate" },
