...
const nodes = [
  ...
  { id: "bandmemory", label: "BandMemory + BLEBands", type: "service", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Services/BandMemory.swift", desc: "Persists the last-used BLE band (UserDefaults, on-device) so it reconnects automatically next launch/workout — no re-picking. BLEBands holds the pure, device-free rules: merge(system-connected + scanned), displayList (synthesize the remembered row when unseen), and bandToAutoConnect (remembered → single system-connected → nil). All unit-tested without CoreBluetooth.", tags: ["ble","persistence","pure","auto-connect"] },
