...
const nodes = [
  ...
  { id: "studio-timeline", label: "StudioTimelineView", type: "component", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/StudioTimelineView.swift", desc: "The edits/CapCut scrubbable timeline: clip strips laid out by output duration, a fixed centre playhead, a time ruler, tap-to-select, and drag-to-trim handles (leading→trimStart, trailing→trimEnd) on the selected clip. Dragging the strip scrubs (seeks the preview + moves the playhead); during playback it auto-advances (offset by vm.currentTime). Trim commits once on drag-end (one undo entry + one rebuild). Zoomable: pps is a computed zoomPps·pinchScale (12…200 pt/s) driven by a MagnifyGesture + −/+ buttons. Strips are coloured placeholders — thumbnail strips are a device-only follow-up.", tags: ["swiftui","timeline","scrub","trim","playhead","zoom","studio"] },
  ...
  { id: "studio-overlay-canvas", label: "StudioOverlayCanvas", type: "component", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/StudioOverlayCanvas.swift", desc: "WYSIWYG overlay editing layer drawn over the studio preview (the edits/CapCut pattern): each text/sticker OverlayItem is a draggable SwiftUI chip — overlays are NOT baked into the live preview video (the Core Animation tool is export-only), so they're live UI here and burned in only at export. Positions against the displayed video rect (ClipEditGeometry.displayRect — aspect-fit inside the player) using the SAME normalized 0..1 position the export layerPoint reads, so a dragged overlay lands at the same spot in the file (WYSIWYG). Drag commits the normalized centre on end (no player rebuild). A .video (picture-in-picture) overlay is composited into the player (a second video track), so its chip is a draggable + pinchable frame outline (the real PiP shows through; position/scale rebuild the composition). Pure SwiftUI ⇒ works on the simulator too.", tags: ["swiftui","overlays","drag","pinch","pip","wysiwyg","studio","pure"] },
  ...
  { id: "studio-overlays", label: "StudioOverlays", type: "service", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Services/StudioOverlays.swift", desc: "S4 overlay renderer: builds a Core Animation layer tree composited via AVVideoCompositionCoreAnimationTool — a CATextLayer per text OverlayItem and a tinted SF-Symbol CALayer per sticker, positioned via ClipEditGeometry.layerPoint, scaled/rotated/coloured, time-gated to [startSec,endSec] and driven by opacityKeyframes when present (else static). Attached by StudioComposer on the instruction paths AND the filter path (overlay-over-filter works) — but ONLY for export: the tool is offline-render-only (AVPlayerItem rejects it), so the live preview skips it (StudioComposer forPlayback). Device-verified export ~4.9s/16s.", tags: ["coreanimation","overlays","text","sticker","studio","device-only"] },
