...
const nodes = [
  ...
  { id: "kilter-create", label: "Create climb (sheet/screen)", type: "sheet", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/CreateClimbView.swift", desc: "Author a brand-new climb by hand (tap holes on the editable board to cycle start→middle→finish→foot) or via the ✨ Generate tab (on-device transformer). On Save it validates against the downloaded dataset + previously-created climbs (KilterDuplicateChecker), assigns a deterministic content uuid (KilterClimbIdentity), and persists a created-climb record. Lights the draft on a connected board (BLE) and exports its p<placement>r<role> frames. iOS CreateClimbView.swift / Android CreateClimbScreen.kt; opened from the browse More menu, surfaced via a Mine filter.", tags: ["authoring","editor","generate","dedup","kilter"] },
  { id: "kilter-identity", label: "KilterClimbIdentity", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterClimbIdentity.swift", desc: "Deterministic, content-addressed climb identity (pure, unit-tested). uuid(layout,frames) is a UUIDv5 (SHA-1, fixed namespace) over the canonical (layout, sorted holds) — the SAME holds on iOS and Android collapse to one uuid (a literal time-based uuid can't). A shared golden vector pins the exact value across platforms. iOS KilterClimbIdentity.swift / Android KilterClimbIdentity.kt.", tags: ["pure","uuid","identity","content-hash","cross-platform"] },
  { id: "kilter-dup-checker", label: "KilterDuplicateChecker", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterDuplicateChecker.swift", desc: "Validates a freshly-authored (manual or generated) climb against everything on the device before save: the downloaded catalog (matched by canonical frames) AND the user's created climbs (matched by the content uuid). Layout-scoped, order-independent. Pure init/find; build() reads the installed catalog. iOS .swift / Android .kt.", tags: ["dedup","validation","pure","kilter"] },
  { id: "kilter-created-climb", label: "KilterCreatedClimb", type: "model", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterCreatedClimb.swift", desc: "A climb the user authored (source manual | generated), keyed by the deterministic content uuid. asClimb adapts it to the read-only KilterClimb so it reuses the whole render/detail/BLE/logging path. iOS SwiftData @Model / Android Room @Entity (kilter_created, DB v4); surfaced via the Mine browse filter.", tags: ["persistence","model","authoring","kilter"] },
  { id: "kilter-generator", label: "KilterClimbGenerator", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterClimbGenerator.swift", desc: "Pure port of the board-explorer's autoregressive decode: conditions on [BOS,SIZE,ANGLE,GRADE,MATCH|NOMATCH], samples hold tokens masked to the board size (no duplicate placement), gates EOS on min-length+start+finish, repairs missing start/finish → valid by construction; linear predicted-grade model + best-of-N. Logits come from an injected provider (the ONNX session in prod, a stub in tests). iOS .swift / Android .kt.", tags: ["pure","generator","transformer","decode","kilter"] },
  { id: "kilter-generator-runtime", label: "Generator runtime (ONNX)", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterGeneratorRuntime.swift", desc: "The ONNX Runtime edge — the only file importing the runtime (iOS OnnxRuntimeBindings / Android ai.onnxruntime). Builds the int64 tokens[1,block] tensor, reads logits[1,block,vocab] at the last token's position; isolates the session off the main thread. Model+meta lazy-downloaded by KilterGeneratorAssets from the Snappet host. iOS .swift / Android KilterGeneratorRuntime.kt.", tags: ["onnx","runtime","ml","edge","kilter"] },
  { id: "ext-onnxruntime", label: "ONNX Runtime", type: "external", group: "platform", category: "platform", platform: "external",
    file: "—", desc: "Microsoft ONNX Runtime. Runs the on-device climb-generator transformer (model.q.onnx). iOS onnxruntime-swift-package-manager (OnnxRuntimeBindings) / Android onnxruntime-android. Only the generator runtime imports it.", tags: ["ml","onnx"] },
