...
const nodes = [
  ...
  { id: "climb-effort", label: "ClimbEffort", type: "engine", group: "engine", category: "core", platform: "engine",
    file: "ios/HighlightEngine/Sources/HighlightEngine/ClimbEffort.swift", desc: "Per-climb effort + recovery over one climb's HR window (the climbing analogue of a single 'burn'): smoothed peak bpm, peak %HRR (only with a real max-HR bound, else nil — the bpm-only state), HR rise + time-to-peak, and HRR60/30 recovery read PAST the window end. Reuses HeartRateSeries for the resample→smooth + %HRR. The caller (KilterSessionStats) extends each climb's window end by HighlightConfig.hrLagSec so a post-effort spike landing just after endedAt is captured — media-assignment climbWindows stays un-extended. Pure, platform-free, swift-test'd.", tags: ["hr","effort","recovery","%hrr","pure","tested"] },
