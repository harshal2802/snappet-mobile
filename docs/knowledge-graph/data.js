...
const nodes = [
  ...
  { id: "kilter-catalog-store", label: "KilterCatalogStore", type: "service", group: "kilter", category: "fitness", platform: "ios+android",
    file: "ios/App/Snappet/Features/Kilter/KilterCatalogStore.swift", desc: "Owns the on-device, user-installed catalog file (Application Support on iOS / filesDir on Android) + a catalog.meta.json sidecar (version / climb count / size). The app ships NO catalog (#42); KilterCatalog opens from here, and when nothing is installed the module shows the opt-in sync screen. install()/clear() + a change notification drive the reader reload.", tags: ["catalog","storage","on-device","#42"] },
  ...
  { id: "kilter-catalog-download", label: "HostedCatalogClient", type: "service", group: "kilter", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/Kilter/KilterAuroraSync.swift", desc: "Phase-2 in-app download (#42, personal/sideload only): GETs a board's gzipped full SQLite from a user-controlled static host (Snappet Board Explorer Pages by default), stream-inflates via zlib, then trims on-device to a small importable catalog — mirroring the explorer's exportDb.ts (ATTACH source, copy reference tables whole, subset climb tables to a filtered + top-N-most-climbed _keep set, VACUUM). No Aurora API, no accounts, nothing uploaded; one GET to the configured host. Aurora's own /sync host rejects all TLS, so this hosted-dataset path replaced it.", tags: ["download","gzip","sqlite","trim","no-aurora-api","#42"] },
