...
const nodes = [
  ...
  { id: "studio-editor", label: "StudioEditorView", type: "cover", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/StudioEditorView.swift", desc: "The full CapCut-style MULTI-CLIP studio (S1): a preview canvas over the live StudioComposer composition, a horizontal timeline of the session's video clips (select · reorder · split · delete), per-clip controls (speed · filter · transition-to-next), project aspect, text overlay, undo/redo, and Export → Share. Editing is model-driven (works on the simulator); preview/export render through PHAsset → AVFoundation and are device-only (the canvas shows a device-only placeholder on the sim). Opened from the summary's media section over the session's StudioProject.", tags: ["editor","studio","multi-clip","timeline","undo","device-only"] },
