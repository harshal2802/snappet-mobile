...
const nodes = [
  ...
  { id: "wt-session-detail", label: "SessionDetailView", type: "screen", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/SessionDetailView.swift", desc: "The enriched post-workout summary: set-by-set breakdown, a live HR chart + avg/max/min and a time-in-zone bar (when the session has an hrSeries), and the tagged-media gallery — now grouped by the set each clip was filmed during (· Set n) with a General bucket for anything not tied to a set, a per-clip Move-to… menu to fix a wrong auto-guess or pin to General (reconciled via SessionMediaAssignment on appear / after discovery), where a video opens the clip editor and \"Generate highlight\" opens the engine-driven reel. Pushed by SessionRoute id (so it can't collide with the live-player cover).", tags: ["detail","summary","hr","media","per-set"], shot: "../screenshots/workout-summary.png" },
