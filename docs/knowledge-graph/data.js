...
const nodes = [
  ...
  { id: "wt-history", label: "History section", type: "section", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/HistorySectionView.swift", desc: "Completed sessions, month-grouped; tap to push session detail, swipe to delete. Searchable by routine name (.searchable) plus a one-tap routine filter chip row (distinct names, most recently trained first; shown only when history spans >1 routine) — filtering is the pure, unit-tested HistorySearch (issue #73).", tags: ["section","search","filter"], shot: "../screenshots/workout-history.png" },
  ...
  { id: "wt-set-editing", label: "SessionSetEditing", type: "model", group: "workout-log", category: "fitness", platform: "ios",
    file: "ios/App/Snappet/Features/WorkoutTracker/SessionSetEditing.swift", desc: "PURE build/apply for the summary's edit-sets mode (issue #73): completed reps/weight sets become text drafts keyed by (exercise id, set index); Save parses them back with the live player's exact input rules (SetMeasure.parseReps/parseWeight — shared, so sanity guards can't drift) and rewrites the exercises array touching ONLY actualReps/actualWeight (completion stamps, units, kinds, set count — and so per-set media keys — survive). Unit-tested incl. an in-memory SwiftData round-trip proving PRs/volume recompute from the corrected stored values (SessionSetEditingTests).", tags: ["pure","edit","tested"] },
