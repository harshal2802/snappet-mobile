...
@Dao
interface KilterDao {
    /** Returns the new row's autogenerated id so an undo can delete exactly this entry. */
    @Insert suspend fun insertLog(entry: KilterLogEntry): Long

    @Query("DELETE FROM kilter_log WHERE id = :id") suspend fun deleteLogById(id: Long)
    ...
    /**
     * The canonical "delete a climb I authored" (Your Climbs P2): removes ONLY the [KilterCreatedClimb]
     * row, never the [KilterLogEntry] ascents logged against it — a real send must never vanish because
     * the climb was tidied up. The two tables are independent (no FK cascade), so a plain row delete keeps
     * History intact. Mirrors the iOS `KilterCreatedClimb.delete` keep-ascents guarantee.
     */
    @Query("DELETE FROM kilter_created WHERE uuid = :uuid")
    suspend fun deleteCreated(uuid: String)

    /** How many ascents the user has logged against a created climb — shown in the delete confirmation so
     *  the keep-ascents guarantee is visible ("your N logged ascents stay in History"). */
    @Query("SELECT COUNT(*) FROM kilter_log WHERE climbUuid = :uuid")
    suspend fun ascentCountForClimb(uuid: String): Int
