...
final class LiveWorkoutOffsetTests: XCTestCase {
    ...
    func testMessageRoundTrips() {
        ...
        let s = LiveWorkoutMessage.start(activityType: HKWorkoutActivityType.climbing.rawValue,
                                         maxHR: nil, restHR: nil)
        XCTAssertEqual(LiveWorkoutMessage(payload: s.payload), s)
        // The Phase-2 maxHR + Phase-4 restHR fields ride the start message and survive the round-trip.
        let sBounds = LiveWorkoutMessage.start(activityType: HKWorkoutActivityType.climbing.rawValue,
                                               maxHR: 188, restHR: 52)
        XCTAssertEqual(LiveWorkoutMessage(payload: sBounds.payload), sBounds)
        XCTAssertNotEqual(sBounds, s)   // the bounds participate in equality
